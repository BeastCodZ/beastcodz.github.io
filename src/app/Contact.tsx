...
const Contact = () => {
  ...
  const showMailIcon = () => {
    ...
    if (buttonRef.current) {
      mailIcon.style.left = `${buttonRef.current.getBoundingClientRect().left + 20}px`;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await emailjs.send("service_8nbcnmh", "template_jl9f62t", emailData, "udACHzHoWl2TbZC-M");

      // Start fade-out effect
      setIsFadingOut(true);
      setTimeout(() => {
        setIsFormVisible(false); // Hide the form after the fade-out animation
        setSubmissionStatus("Message Sent!");
      }, 300); // Match this with the duration of the fade effect
    } catch (error) {
  ...
  return (
    <section id="contact" className="min-h-screen flex flex-col justify-center items-center bg-gray-900 text-white px-8 py-16">
      {isFormVisible && (
        <motion.form
          ...
          className={`w-full max-w-md mx-auto bg-gray-800 p-8 rounded-lg text-black shadow-lg transition-all duration-300 ${formSize === "small" ? "h-20" : ""}`}
        >
          <h2 className="text-4xl text-white font-bold mb-8 text-center">Get in Touch</h2>
          <motion.input
            type="text"
            name="from_name"
...
/>
  ...
  whileTap={{ scale: 0.9 }}
  className={`relative inline-flex items-center justify-center p-4 overflow-hidden font-medium text-white transition duration-300 ease-out border-2 border-blue-700 rounded-full shadow-md group w-full h-12 ${isSubmitting ? "opacity-50 cursor-not-allowed" : "bg-gray-600 hover:bg-blue-700"}`}
>
  ...
  <span className="relative invisible">{isSubmitting ? "Sending..." : "Send Message"}</span>
</motion.button>

        </motion.form>
      )}
      <div className="bg-gray-800 rounded-xl">
      {submissionStatus && <div className="bg-gray-800 rounded-xl p-3"><p className="text-l text-green-500">{submissionStatus}</p></div>}
